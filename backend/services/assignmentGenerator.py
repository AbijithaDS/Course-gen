...
def parse_assignment_markdown(md_text):
    lines = md_text.split('\n')
    questions = []
    for line in lines:
        line_str = line.strip()
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python assignmentGenerator.py <templatePath> <inputPath> <outputPath>")
        sys.exit(1)
    ...
    questions_data = parse_assignment_markdown(content)
    ...
    for p in doc.paragraphs:
        if "{{ASSIGNMENT_QUESTIONS_PLACEHOLDER}}" in p.text:
            placeholder_p = p
            break
